...
def main():
    ...
    parser.add_argument("--arch", type=str, choices=['vgg16', 'densenet121'], default='vgg16', 
    ...
    parser.add_argument("--load_from", type=str, help="Path to checkpoint to resume training")
    ...
    if args.load_from:
        model = load_checkpoint(args.load_from, model)
        print(f"Resumed training from checkpoint: {args.load_from}")
    ...
    if args.save_dir:
        os.makedirs(args.save_dir, exist_ok=True)  # Create directory if it doesn't exist
        checkpoint = {
